...
def findAll(tree, xpath, parser):
    """A light wrapper around antlr's XPath API
    """
    if parser is None:
        # parser might be none if we are using the C++ parser.
        # For xpaths that only contain rule names, this is fine, but it will break for other cases
        # See below for more info on XPath implementation:
        # https://github.com/antlr/antlr4/blob/master/runtime/Python3/src/antlr4/xpath/XPath.py
        parser = modelicaParser
    
    return XPath.XPath.findAll(tree, xpath, parser)
...
class Selector(ABC):
    ...
    def apply_to_root(self, tree_root, parser):
        ...
        """
        bases = findAll(tree_root, self.BASE_PATH, parser)
        result = []
        for base in bases:
...
class ComponentDeclarationByTypeSelector(Selector):
    ...
    def _select(self, base, parser):
        ...
        return findAll(base, xpath, parser)
...
class ComponentDeclarationSelector(Selector):
    ...
    def _select(self, base, parser):
        ...
        xpath = 'component_clause/component_list/component_declaration'
        component_declaration_nodes = findAll(base, xpath, parser)
...
class ComponentArgumentValueSelector(Selector):
    ...
    def _select(self, base, parser):
        ...
        element_modifications = findAll(base, xpath, parser)
        ...
        for element_modification in filtered_modifications:
            xpath = '//expression'
            results.extend(findAll(element_modification, xpath, parser))
